...
const canIssueSpare = computed(() => auth.hasPermission('inventory:issue') && canProcess.value && order.value?.status === 'PROCESSING')
  ...
  <section class="module-page" v-loading="loading">
    ...
    <template v-if="order">
      ...
      <el-card v-if="canProcess && ['ASSIGNED', 'PROCESSING', 'SUSPENDED'].includes(order.status)" shadow="never" class="action-card">
        ...
        <div class="workflow-actions"><el-button v-if="order.status === 'ASSIGNED' && !order.acceptedAt" :loading="actionLoading" @click="perform(() => api.acceptResponse(order.id), '接单成功')">确认接单</el-button><el-button v-if="order.status === 'ASSIGNED'" type="primary" :loading="actionLoading" @click="perform(() => api.start(order.id), '已开始维修，设备进入维修中')">开始维修</el-button><el-button v-if="order.status === 'PROCESSING'" :disabled="actionLoading" @click="openRecord">填写维修记录</el-button><el-button v-if="canIssueSpare" type="primary" plain :disabled="actionLoading" @click="openSpareDialog">领用备件</el-button><el-button v-if="order.status === 'PROCESSING'" type="warning" plain :disabled="actionLoading" @click="reasonAction('suspend')">挂起</el-button><el-button v-if="order.status === 'SUSPENDED'" type="primary" :loading="actionLoading" @click="reasonAction('resume')">恢复处理</el-button><el-button v-if="order.status === 'PROCESSING'" type="primary" :loading="actionLoading" @click="submit">提交验收</el-button></div>
      </el-card>
