...
| 一次修复率 | 同一设备、同一标准故障类别在观察窗口内未再次发生的比例 | 目标公式为 `观察窗内未重复故障的首次维修数 / 可判定首次维修数 × 100%` | 现有 `mnt_repair_request.fault_description` 是自由文本；系统没有标准故障分类字段或重复故障关联字段 | 目标观察窗为维修完成后 7 天，并受查询结束日期限制 | **数据模型缺口**：自由文本不能可靠等同故障类型。本阶段接口返回 `available=false`，不使用“完成工单数/总工单数”冒充一次修复率；后续需增加标准故障分类后再启用计算 |
...
2. 一次修复率缺少标准故障分类或重复故障关联，必须显示“数据模型缺口”。
