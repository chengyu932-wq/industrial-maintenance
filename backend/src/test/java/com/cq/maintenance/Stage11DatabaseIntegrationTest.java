...
class Stage11DatabaseIntegrationTest {
    ...
    @Test void firstTimeFixIsExplicitlyUnavailable(){supervisorLogin();var value=statistics.kpis(query()).metrics().stream().filter(v->"FIRST_TIME_FIX_RATE".equals(v.code())).findFirst().orElseThrow();assertFalse(value.available());assertNull(value.value());assertTrue(value.note().contains("数据模型缺口"));}
