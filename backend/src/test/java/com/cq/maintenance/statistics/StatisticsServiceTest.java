...
class StatisticsServiceTest {
    ...
    @BeforeEach void setUp() {
        ...
        when(mapper.repairAggregate(any(),any(),any())).thenReturn(new RepairAggregate(0,0,0,0));
    ...
    @Test void calculatesAvailableKpisFromMapperAggregates(){when(mapper.countCompletedRepairs(eq(allScope),any(),any())).thenReturn(4L);when(mapper.runtimeHours(eq(allScope),any(),any())).thenReturn(new BigDecimal("120"));when(mapper.repairAggregate(eq(allScope),any(),any())).thenReturn(new RepairAggregate(2,14400,4,3));when(mapper.availability(eq(allScope),any(),any())).thenReturn(new AvailabilityAggregate(80,100));when(mapper.inventoryTurnover(eq(true),anyList(),any(),any())).thenReturn(new InventoryAggregate(new BigDecimal("20"),new BigDecimal("10"),2));var values=service.kpis(custom("2026-09-01","2026-09-07")).metrics();assertEquals(new BigDecimal("30.00"),metric(values,"MTBF").value());assertEquals(new BigDecimal("2.00"),metric(values,"MTTR").value());assertEquals(new BigDecimal("75.00"),metric(values,"ON_TIME_CLOSE_RATE").value());assertEquals(new BigDecimal("80.00"),metric(values,"EQUIPMENT_AVAILABILITY").value());assertEquals(new BigDecimal("2.00"),metric(values,"SPARE_PART_TURNOVER").value());}
    ...
    @Test void firstTimeFixExposesFaultClassificationGap(){var value=metric(service.kpis(custom("2026-09-01","2026-09-07")).metrics(),"FIRST_TIME_FIX_RATE");assertFalse(value.available());assertNull(value.value());assertTrue(value.note().contains("数据模型缺口"));assertTrue(value.formula().contains("7 天"));}
    @Test void invalidRepairTimestampSamplesAreExcludedByAggregateSample(){when(mapper.repairAggregate(eq(allScope),any(),any())).thenReturn(new RepairAggregate(0,0,2,1));var values=service.kpis(custom("2026-09-01","2026-09-07")).metrics();assertFalse(metric(values,"MTTR").available());assertEquals(0,metric(values,"MTTR").sampleSize());assertEquals(new BigDecimal("50.00"),metric(values,"ON_TIME_CLOSE_RATE").value());}
